...
$download_url = portal_get_attach_first($aid);
// var_dump($download_url);die;
...
/**
 * add by Daming 2015/10/25 获取文章中的最后一个可以下载附件的下载连接
 * @param number $aid
 */
function portal_get_attach_first($aid) {
	$attach = C::t('portal_attachment')->fetch_all_by_aid($aid);
	$url = '';
	foreach ($attach as $val) {
		if ($val['filetype'] == 'attach') {
			$url .= 'portal.php?mod=attachment&id='.$val['attachid'];		
		}
	}
	return $url;
}
